...
class linear_layer:
    ...
    def forward(self, X):
        ...
        # note that forward(𝑥)=𝑊(1)𝑥+𝑏(1) W: (D,D) X:(N,D) b:(1,D)
...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        # store elements that are < 0 into mask
        self.mask = X < 0

        forward_output = X
        # 𝑚𝑎𝑥{0,𝑢}, if < 0 then max must be zero
    ...
    def backward(self, X, grad):
        ...
        """
            ...
            Return:
            - backward_output: A numpy array of the same shape as X, where each element is the partial derivative of
            the mini-batch loss w.r.t. the corresponding element in  X.
        """
        ...
        # gradient = 0 if u < 0 since 𝑚𝑎𝑥{0,𝑢}
        backward_output[self.mask] = 0.0
...
def miniBatchGradientDescent(model, momentum, _lambda, _alpha, _learning_rate):
    ...
    for module_name, module in model.items():
        ...
        # module_name is the key, module is the value
